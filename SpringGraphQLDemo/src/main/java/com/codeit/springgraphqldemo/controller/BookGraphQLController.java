...
public class BookGraphQLController {
  ...
  public BookGraphQLController() {
    ...
    books.add(new Book(sequence++, "자바 입문", "Codeit", 25000));
    books.add(new Book(sequence++, "Spring Boot 기초", "Codeit", 32000));
  }
  ...
  public Book createBook(@Argument CreateBookInput input) {
    printLog("createBook Mutation 실행: title=" + input.title());

    Book book = new Book(sequence++, input.title(), input.author(), input.price());
    books.add(book);
    return book;
  }
