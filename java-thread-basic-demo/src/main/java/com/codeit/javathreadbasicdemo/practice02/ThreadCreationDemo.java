...
public class ThreadCreationDemo {
  ...
  private static class CountingThread extends Thread{
    ...
    public CountingThread(String taskName, int start, int end, int delayMills) {
    ...
    public void run() {
      ...
      for (int i = start; i <= end; i++) {
        System.out.println(  "[" + getName() + "] 현재 값: "
            + i);
        try {
  ...
  private static class CountingTask implements Runnable{
    ...
    public CountingTask(String taskName, int start, int end, int delayMills) {
      this.taskName = taskName;
      this.start = start;
      this.end = end;
      this.delayMills = delayMills;
