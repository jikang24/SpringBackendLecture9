...
public class SyncAsyncComparisonDemo {
  ...
  private static void rucSynchronousProcessing(){
    ...
    System.out.println(
        "[동기 처리 종료] 총 소요 시간: " + (endTime - startTime) + "ms"
    );
  ...
  private static void downloadFile(
    ...
    try {
      System.out.println("[" + threadName + "] 다운로드 시작: "
          + fileName);

      Thread.sleep(delayMills);

      System.out.println( "[" + threadName + "] 다운로드 완료: "
          + fileName);
    }
    catch (InterruptedException e) {
      System.out.println( "[" + threadName + "] 다운로드 중 인터럽트 발생: "
          + fileName
      );
